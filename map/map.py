...
class Map:
    ...
    def __generate_vertices__(self) -> None:
        for y in range(self.__height + 1):
            for x in range(self.__width + 1):
                ...
                node_name = NODE_NAME[index] if index < len(
                    NODE_NAME) else str(index)
    ...
    def __assign_node_neighbors__(self) -> None:
        for i in range((self.__width + 1) * (self.__height + 1)):
            self.__nodes[i].top_node = self.__nodes[
                i - self.__width - 1] if i - self.__width - 1 >= 0 else None
            self.__nodes[i].bottom_node = self.__nodes[
                i + self.__width + 1] if i + self.__width + 1 < (
                    self.__width + 1) * (self.__height + 1) else None
            self.__nodes[i].left_node = self.__nodes[
                i - 1] if i % (self.__width + 1) != 0 else None
            self.__nodes[i].right_node = self.__nodes[
                i + 1] if i % (self.__width + 1) != self.__width else None

    def __assign_grid_vertices__(self) -> None:
        for y in range(self.__height + 1):
            for x in range(self.__width + 1):
                self.__nodes[y * (self.__width + 1) +
                             x].top_left_place = self.__grids[y - 1][
                                 x - 1] if y - 1 >= 0 and x - 1 >= 0 else None
                self.__nodes[
                    y * (self.__width + 1) + x].top_right_place = self.__grids[
                        y - 1][x] if y - 1 >= 0 and x < self.__width else None
                self.__nodes[
                    y * (self.__width + 1) +
                    x].bottom_left_place = self.__grids[y][
                        x - 1] if x - 1 >= 0 and y < self.__height else None
                self.__nodes[
                    y * (self.__width + 1) +
                    x].bottom_right_place = self.__grids[y][
                        x] if x < self.__width and y < self.__height else None
