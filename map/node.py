    ...
    def set_neighbors_distance(self) -> None:
        ...
        if self.top_node != None:
            if self.left_node != None:
                if self.right_node != None:  # Left & Right
                    cost: float = (
                        self.top_left_place.get_cost()
                        + self.top_right_place.get_cost() / 2
                    )
                    self.__neighbors[self.top_node] = (
                        cost if cost < 3.0 else float("inf")
                    )
                else:  # left & !Right
        ...
        if self.bottom_node != None:
            if self.left_node != None:
                if self.right_node != None:  # Left & Right
                    cost: float = (
                        self.bottom_left_place.get_cost()
                        + self.bottom_right_place.get_cost() / 2
                    )
                    self.__neighbors[self.bottom_node] = (
                        cost if cost < 3.0 else float("inf")
                    )
                else:  # left & !Right
                    self.__neighbors[
        ...
        if self.left_node != None:
            if self.top_node != None:
                if self.bottom_node != None:  # Top & Bot
                    ...
                    self.__neighbors[self.left_node] = (
                        cost if cost < 3.0 else float("inf")
                    )
                else:  # Top & !Bot
        ...
        if self.right_node != None:
            if self.top_node != None:
                if self.bottom_node != None:  # Top & Bot
                    cost: float = (
                        self.top_right_place.get_cost()
                        + self.bottom_right_place.get_cost() / 2
                    )
                    self.__neighbors[self.right_node] = (
                        cost if cost < 3.0 else float("inf")
                    )
                else:  # Top & !Bot
